...
class PodcastRoomController extends Controller
{
    ...
    public function calculateOccupiedHours($date, $roomName = null, $excludeId = null)
    {
        ...
        $bookings = PodcastRoomBooking::whereDate('date', $date)
        ...
        $logsQuery = RoomUsageLog::where('room_type', 'podcast_room')
        ...
        if ($roomName) {
            $logsQuery->whereIn('reservation_id', function($sub) use ($roomName) {
                $sub->select('id')->from('podcast_room_bookings')->where('room_name', $roomName);
            });
        }
    ...
    public function adminIndex(Request $request)
    {
        ...
        foreach ($activePodcastBenefits as $bnf) {
            ...
            if ($bnf->podcast_room_booking_id) {
                $existingBooking = PodcastRoomBooking::find($bnf->podcast_room_booking_id);
            }
            if (!$existingBooking) {
                $existingBooking = PodcastRoomBooking::where('benefit_id', $bnf->id)->first();
            }
            ...
            if ($existingBooking) {
                ...
                if ($bnf->podcast_room_booking_id !== $existingBooking->id) {
                    $bnf->update(['podcast_room_booking_id' => $existingBooking->id]);
                }
                continue;
            }
    ...
    public function createSession(Request $request)
    {
        ...
        for ($h = $sh; $h < $eh; $h++) {
            ...
            if (in_array($reqSlot, $occupiedSlots)) {
                return back()->withInput()->with('error', "🚫 Jam {$reqSlot} pada studio {$request->room_name} sudah digunakan atau dibooking oleh client lain.");
            }
        }
    ...
    public function approvePayment($id)
    {
        ...
        if ($booking->user_id && ($booking->duration >= 10 || empty($booking->date))) {
            $benefit = RoomBenefit::where('podcast_room_booking_id', $booking->id)
    ...
    public function checkin(Request $request, $id)
    {
        ...
        if ($startTime) {
            ...
            for ($h = $sh; $h < $eh; $h++) {
                $reqSlot = sprintf('%02d:00', $h);
                if (in_array($reqSlot, $occupiedSlots)) {
                    return back()->withInput()->with('error', "🚫 Jam {$reqSlot} pada studio {$roomName} sudah digunakan atau dibooking oleh client lain.");
                }
            }
        }
    ...
    private function getActiveBenefit(): ?RoomBenefit
    {
        ...
        if ($packageBooking && $packageBooking->remaining_seconds > 0) {
            $existing = RoomBenefit::where('podcast_room_booking_id', $packageBooking->id)
