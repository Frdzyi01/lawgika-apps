...
class PodcastRoomRenewalReminder extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'podcast-room:renewal-reminder';
     ...
     * @var string
     */
    protected $description = 'Kirim WhatsApp reminder renewal (H-30, H-7, & Hari H Expired) ke client Paket Studio Podcast.';
    ...
    {
        $this->info('=== Studio Podcast Package Renewal Reminder (H-30, H-7, & Expired) ===');
        ...
        foreach ($benefitsH30 as $benefit) {
            ...
            try {
                $log = $whatsAppService->notifyPodcastRoomRenewalReminderH30($benefit);
                ...
                Log::error('PodcastRoomRenewalReminder H30 - Exception', [
        ...
        foreach ($benefitsH7 as $benefit) {
            ...
            try {
                $log = $whatsAppService->notifyPodcastRoomRenewalReminderH7($benefit);
                ...
                Log::error('PodcastRoomRenewalReminder H7 - Exception', [
                    'benefit_id' => $benefit->id,
                    'error'      => $e->getMessage(),
                ]);
        ...
        foreach ($benefitsExpired as $benefit) {
            ...
            try {
                $log = $whatsAppService->notifyPodcastRoomExpired($benefit);
                ...
                Log::error('PodcastRoomRenewalReminder Expired - Exception', [
                    'benefit_id' => $benefit->id,
                    'error'      => $e->getMessage(),
                ]);
