...
.addon-modal-row.active .addon-btn-toggle {
...
<div class="container-fluid py-2">
    ...
    <div class="admin-page-header">
        <div>
            <h1 class="admin-page-title">
                <i class="fa-solid fa-microphone-lines text-primary"></i> Manajemen Podcast Room
            </h1>
    ...
    @php
        ...
        $activeCheckinCount = \App\Models\PodcastRoomBooking::where('status', 'checkin')->count();
    ...
    <div class="kpi-grid">
        ...
        <div class="kpi-card">
            <div>
                ...
                <div class="kpi-info-val text-success">{{ $activeCheckinCount }} <small class="text-muted fw-normal fs-6">Studio Aktif</small></div>
            </div>
        ...
        <div class="p-0">
                            ...
                            <tr>
                                <td class="ps-4">
                                    <span class="order-code-badge">
                                        {{ $p->order_number ?? ('#POD-' . str_pad($p->id, 5, '0', STR_PAD_LEFT)) }}
                                    </span>
                                ...
                                <td>
                                    ...
                                    @endif
                                    @if($p->notes)
                                        <div class="client-sub-detail mt-1 text-dark">
                                            <span class="badge bg-light text-dark border"><i class="fa-solid fa-sliders me-1"></i> {{ Str::limit($p->notes, 35) }}</span>
                                        </div>
                                    @endif
                                </td>
                                <td>
                                ...
                                <td>
                                    ...
                                    <div class="client-sub-detail">{{ $p->podcast_title ?? '-' }}</div>
                                </td>
                                <td>
                                    @if($p->total_price > 0)
                                    ...
                                    <div class="btn-action-group">
                                        @if($p->source_type === 'benefit')
                                            ...
                                            <form action="{{ url('admin/podcast-room/' . $p->id . '/benefit-approve') }}" method="POST" class="d-inline">
                                                @csrf
                                                <button type="submit" class="btn-approve" onclick="return confirm('Setujui pengajuan reservasi podcast ini?')">
                                                    <i class="fa-solid fa-check"></i> Setujui
                                                </button>
                                            </form>
                                            {{-- Reject Benefit --}}
                                            <form action="{{ url('admin/podcast-room/' . $p->id . '/benefit-reject') }}" method="POST" class="d-inline">
                                                @csrf
                                        ...
                                        @else
                                            ...
                                            <form action="{{ url('admin/podcast-room/' . $p->id . '/approve-payment') }}" method="POST" class="d-inline">
                                                @csrf
                                                <button type="submit" class="btn-approve" onclick="return confirm('Setujui pembayaran & reservasi podcast ini? Kuota akan otomatis aktif.')">
                                                    <i class="fa-solid fa-check"></i> Setujui
                                                </button>
                                            </form>
                                            {{-- Reject Manual Payment --}}
                                            <form action="{{ url('admin/podcast-room/' . $p->id . '/reject-payment') }}" method="POST" class="d-inline">
                                                @csrf
                                        ...
                                        @endif
                                        <a href="{{ url('admin/podcast-room/' . $p->id . '/detail') }}" class="btn-table-detail" title="Lihat Detail">
            ...
            @else
                <div class="empty-box-state">
                    ...
                    <p class="text-muted small mb-0">Reservasi studio podcast baru yang diajukan oleh client akan otomatis tampil di sini.</p>
                </div>
            @endif
        </div>
    </div>
            ...
            <div class="d-flex align-items-center gap-2">
                <form action="{{ url()->current() }}" method="GET" class="d-flex">
                    <div class="input-group input-group-sm">
                        ...
                        <input type="text" name="search" class="form-control form-control-sm border-start-0 ps-0" placeholder="Cari client, order, studio..." value="{{ request('search') }}" style="min-width:210px;">
                        @if(request('search'))
                ...
                <table class="table table-modern">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="ps-4">
                                <span class="order-code-badge">
                                    {{ $b->order_number ?? ('#POD-' . str_pad($b->id, 5, '0', STR_PAD_LEFT)) }}
                                </span>
                            </td>
                            <td>
                            ...
                            <td>
                                @if(!empty($b->start_time) && !empty($b->room_name))
                                    <span class="badge bg-light text-dark border px-2 py-1 fw-semibold">
                                        <i class="fa-solid fa-microphone-lines text-primary me-1"></i> {{ $b->room_name }}
                                    </span>
                                    @if($b->podcast_title)
                                        <div class="client-sub-detail mt-0.5">"{{ $b->podcast_title }}"</div>
                                    @endif
                                @else
                            ...
                            <td>
                                ...
                                @if($b->notes)
                                    <div class="client-sub-detail mt-1">
                                        <span class="badge bg-light text-dark border"><i class="fa-solid fa-sliders me-1"></i> {{ Str::limit($b->notes, 30) }}</span>
                                    </div>
                                @endif
                            </td>
                            <td>
                            ...
                            <td class="pe-4 text-end">
                                <div class="btn-action-group">
                                    <a href="{{ url('admin/podcast-room/'.$b->id.'/detail') }}" class="btn-table-detail">
                                        <i class="fa-solid fa-eye"></i> Detail
                                    </a>
                                    ...
                                    @if(empty($b->start_time) && $b->remaining_seconds > 0 && ($b->status === 'approved' || $b->payment_status === 'approved'))
                                        <button type="button" class="btn btn-sm text-white px-2.5 py-1 fw-bold rounded-2 shadow-sm" data-bs-toggle="modal" data-bs-target="#createSessionModalPR{{ $b->id }}" style="font-size:0.78rem; background:#4e0516; border-color:#4e0516;">
                                                    ...
                                                    <div class="modal-body">
                                                        ...
                                                        <div class="row">
                                                            ...
                                                            <div class="col-md-6 mb-3">
                                                                ...
                                                                <input type="text" name="podcast_title" class="form-control" value="{{ $b->podcast_title ?? '' }}" placeholder="Misal: Sesi Diskusi Bisnis...">
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="card border-0 bg-light rounded-3 p-3 mb-3">
                                                            ...
                                                            <div class="d-flex flex-column gap-2">
                                                                ...
                                                                <div class="addon-modal-row" id="row_mic_{{ $b->id }}" onclick="toggleModalAddon('{{ $b->id }}', 'mic', 50000)">
                                                                    <div class="d-flex align-items-center gap-2">
                                                                        <div class="addon-modal-icon"><i class="fa-solid fa-microphone-lines"></i></div>
                                                                        <div>
                                                                            <div class="fw-bold text-dark" style="font-size:0.84rem;">Mikrofon Tambahan <span class="badge bg-secondary text-white px-1.5 py-0.5" style="font-size:0.65rem;">Maks. 1</span></div>
                                                                            <div class="text-muted" style="font-size:0.75rem;">+ Rp 50.000 / jam</div>
                                                                        </div>
                                                                    </div>
                                                                    <div>
                                                                        <button type="button" class="btn btn-sm btn-outline-secondary rounded-pill px-3 py-1 fw-bold addon-btn-toggle" id="btn_toggle_mic_{{ $b->id }}" style="font-size:0.75rem;">
                                                                            <i class="fa-solid fa-plus me-1"></i> <span>Pilih</span>
                                                                        </button>
                                                                        <input type="hidden" name="addon_mic" id="addon_mic_{{ $b->id }}" value="0">
                                                                    </div>
                                                                </div>

                                                                <!-- 2. Headphone (+1 Unit Maks) -->
                                                                <div class="addon-modal-row" id="row_headphone_{{ $b->id }}" onclick="toggleModalAddon('{{ $b->id }}', 'headphone', 50000)">
                                                                    <div class="d-flex align-items-center gap-2">
                                                                        <div class="addon-modal-icon"><i class="fa-solid fa-headphones-simple"></i></div>
                                                                        <div>
                                                                            <div class="fw-bold text-dark" style="font-size:0.84rem;">Headphone Tambahan <span class="badge bg-secondary text-white px-1.5 py-0.5" style="font-size:0.65rem;">Maks. 1</span></div>
                                                                            <div class="text-muted" style="font-size:0.75rem;">+ Rp 50.000 / jam</div>
                                                                        </div>
                                                                    </div>
                                                                    <div>
                                                                        <button type="button" class="btn btn-sm btn-outline-secondary rounded-pill px-3 py-1 fw-bold addon-btn-toggle" id="btn_toggle_headphone_{{ $b->id }}" style="font-size:0.75rem;">
                                                                            <i class="fa-solid fa-plus me-1"></i> <span>Pilih</span>
                                                                        </button>
                                                                        <input type="hidden" name="addon_headphone" id="addon_headphone_{{ $b->id }}" value="0">
                                                                    </div>
                                                                </div>

                                                                <!-- 3. Kamera Tambahan (Maks 2 Unit) -->
                                                                <div class="addon-modal-row" id="row_camera_{{ $b->id }}">
                                                                    <div class="d-flex align-items-center gap-2">
                                                                        ...
                                                                        <div>
                                                                            ...
                                                                            <div class="text-muted" style="font-size:0.75rem;">+ Rp 150.000 / unit / jam</div>
                                                                        </div>
                                                                    </div>
                                                                    <div class="d-flex align-items-center gap-1 bg-white border rounded-pill px-2 py-0.5" onclick="event.stopPropagation()">
                                                                        <button type="button" class="btn btn-sm p-0 text-muted fw-bold" style="width:20px; height:20px;" onclick="changeModalCamQty('{{ $b->id }}', -1)" id="btn_cam_minus_{{ $b->id }}" disabled>−</button>
                                                                        <span class="fw-bold px-2 text-dark" id="val_addon_camera_{{ $b->id }}" style="font-size:0.85rem;">0</span>
                                                                        <button type="button" class="btn btn-sm p-0 text-dark fw-bold" style="width:20px; height:20px;" onclick="changeModalCamQty('{{ $b->id }}', 1)" id="btn_cam_plus_{{ $b->id }}">+</button>
                                                                        <input type="hidden" name="addon_camera" id="addon_camera_{{ $b->id }}" value="0">
                                                                    </div>
                                                                </div>

                                                                <!-- 4. Operator Podcast (Maks 1) -->
                                                                <div class="addon-modal-row" id="row_operator_{{ $b->id }}" onclick="toggleModalAddon('{{ $b->id }}', 'operator', 100000)">
                                                                    <div class="d-flex align-items-center gap-2">
                                                                    ...
                                                                    <div>
                                                                        <button type="button" class="btn btn-sm btn-outline-secondary rounded-pill px-3 py-1 fw-bold addon-btn-toggle" id="btn_toggle_operator_{{ $b->id }}" style="font-size:0.75rem;">
                                                                            <i class="fa-solid fa-plus me-1"></i> <span>Pilih</span>
                                                                        </button>
                                                                        <input type="hidden" name="addon_operator" id="addon_operator_{{ $b->id }}" value="0">
                                                                    </div>
                                                                </div>
                                                            </div>
                                    ...
                                    @if(!empty($b->start_time) && ($b->status === 'approved' || $b->status === 'paused' || $b->payment_status === 'approved') && $b->status !== 'checkin' && $b->status !== 'selesai' && $b->status !== 'rejected' && $b->status !== 'pending')
                                        <button type="button" class="btn btn-sm btn-success px-2.5 py-1 fw-bold rounded-2 shadow-sm" data-bs-toggle="modal" data-bs-target="#checkinModalPR{{ $b->id }}" style="font-size:0.78rem;">
                                            <i class="fa-solid fa-door-open me-1"></i> Check In
                                        </button>
                                        <button type="button" class="btn btn-sm btn-outline-secondary px-2 py-1 rounded-2" data-bs-toggle="modal" data-bs-target="#createSessionModalPR{{ $b->id }}" title="Ubah Jadwal Reservasi" style="font-size:0.75rem;">
                                                        ...
                                                        <div class="modal-body">
                                                            ...
                                                            <div class="mb-3">
                                                                ...
                                                                <select name="room_name" class="form-select" required onchange="refreshModalSlots(this.closest('.modal'), '{{ url('admin/podcast-room/booked-slots') }}', '{{ $b->id }}')">
                                                                    ...
                                                                    @endphp
                                                                    @foreach($prRooms as $rm)
                                                                        @php
                                                                            $isOccupiedByOther = \App\Models\PodcastRoomBooking::where('room_name', $rm)
                                    ...
                                    @elseif($b->status === 'checkin')
                                        <form action="{{ url('admin/podcast-room/'.$b->id.'/checkout') }}" method="POST" class="d-inline">
                                            @csrf
                                            <button class="btn btn-sm btn-warning text-dark px-2.5 py-1 fw-bold rounded-2" onclick="return confirm('Lakukan Check Out untuk sesi studio podcast ini?')" style="font-size:0.78rem;">
                        ...
                        @empty
                        <tr>
                            <td colspan="9" class="text-center py-4 text-muted">Belum ada data reservasi studio podcast.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
...
@push('scripts')
    <script>
        ...
        /* Add-On Modal Interactive Helpers */
        function toggleModalAddon(bookingId, key, pricePerHour) {
            const inputEl = document.getElementById(`addon_${key}_${bookingId}`);
            const rowEl   = document.getElementById(`row_${key}_${bookingId}`);
            const btnEl   = document.getElementById(`btn_toggle_${key}_${bookingId}`);
            ...
            checkModalAddonPaymentBox(bookingId);
        }

        function changeModalCamQty(bookingId, delta) {
            const inputEl = document.getElementById(`addon_camera_${bookingId}`);
            const valEl   = document.getElementById(`val_addon_camera_${bookingId}`);
            const rowEl   = document.getElementById(`row_camera_${bookingId}`);
            const btnMinus = document.getElementById(`btn_cam_minus_${bookingId}`);
            const btnPlus  = document.getElementById(`btn_cam_plus_${bookingId}`);
        ...
        function checkModalAddonPaymentBox(bookingId) {
            const mic = parseInt(document.getElementById(`addon_mic_${bookingId}`)?.value || 0);
            const hp  = parseInt(document.getElementById(`addon_headphone_${bookingId}`)?.value || 0);
            const cam = parseInt(document.getElementById(`addon_camera_${bookingId}`)?.value || 0);
            const op  = parseInt(document.getElementById(`addon_operator_${bookingId}`)?.value || 0);

            const box = document.getElementById(`payment_addon_box_${bookingId}`);
            if (box) {
