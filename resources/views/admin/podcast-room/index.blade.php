                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @else
                                    <span class="badge bg-info text-dark">Benefit</span>
                                @endif
                            </td>
                            <td>
                                @if($booking->payment_proof)
                                    <a href="{{ asset('storage/'.$booking->payment_proof) }}" target="_blank">
                                        <img src="{{ asset('storage/'.$booking->payment_proof) }}"
                                            style="width:65px;height:65px;object-fit:cover;border-radius:8px;border:1px solid #e2e8f0;"
                                            title="Klik untuk perbesar">
                                    </a>
                                    @if($booking->payment_status === 'pending')
                                        <div class="mt-1 d-flex gap-1">
                                            <form action="{{ url('admin/podcast-room/'.$booking->id.'/approve-payment') }}" method="POST">
                                                @csrf<button class="btn btn-xs btn-success py-0 px-2" style="font-size:.72rem;" onclick="return confirm('Setujui pembayaran ini?')">✅ OK</button>
                                            </form>
                                            <form action="{{ url('admin/podcast-room/'.$booking->id.'/reject-payment') }}" method="POST">
                                                @csrf<button class="btn btn-xs btn-danger py-0 px-2" style="font-size:.72rem;" onclick="return confirm('Tolak pembayaran ini?')">❌</button>
                                            </form>
                                        </div>
                                    @elseif($booking->payment_status === 'approved')
                                        <span class="badge bg-success d-block mt-1" style="font-size:.7rem;">✅ Lunas</span>
                                    @elseif($booking->payment_status === 'rejected')
                                        <span class="badge bg-danger d-block mt-1" style="font-size:.7rem;">❌ Ditolak</span>
                                    @endif
                                    @else
                                        <span class="badge bg-success" style="font-size:.72rem;">✅ Gratis</span>
                                        <small class="d-block text-muted mt-1" style="font-size:.72rem;">Benefit Paket – tidak perlu bukti bayar</small>
                                    @endif
                            </td>
                            <td>
                                @if($booking->benefit)
                        ...
                        @empty
                        <tr>
                            <td colspan="10" class="text-center text-muted">Belum ada reservasi benefit ruang podcast.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover">
                    ...
                    <tbody>
                        ...
                        @empty
                        <tr>
                            <td colspan="10" class="text-center">Belum ada data reservasi ruang podcast.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
