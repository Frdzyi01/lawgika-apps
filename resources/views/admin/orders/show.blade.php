...
@section('content')
<style>
.doc-type-card { border-radius:12px; border:1.5px solid #e5e7eb; overflow:hidden; margin-bottom:1rem; }
.doc-type-header { background:#f8fafc; padding:12px 16px; border-bottom:1px solid #e5e7eb; }
.doc-file-row { padding:10px 16px; border-bottom:1px solid #f1f5f9; transition:background .15s; }
.doc-file-row:last-child { border-bottom:0; }
.doc-file-row:hover { background:#f8fafc; }
.status-dot { width:10px;height:10px;border-radius:50%;display:inline-block; }
.order-detail-table td { color:inherit !important; }
</style>
...
<div class="row g-4">
        ...
        <div class="card border-0 shadow-sm mb-4">
            ...
            <div class="card-body px-4">
                <table class="table table-borderless text-body order-detail-table">
                    <tr><td width="40%" class="text-muted">No. Pesanan</td>
                        <td class="fw-semibold">{{ $order->order_number }}</td></tr>
                    <tr><td class="text-muted">Layanan</td>
                        <td class="fw-semibold">{{ $order->service_name ?? ($order->service->name ?? '—') }}</td></tr>
                    <tr><td class="text-muted">Customer</td>
                        <td>{{ $order->user->name ?? '—' }}<br>
                            <small class="text-muted">{{ $order->user->email ?? '' }}</small><br>
                            <small class="text-muted">{{ $order->user->phone ?? '' }}</small></td></tr>
                    <tr><td class="text-muted">Tanggal</td>
                        <td>{{ $order->created_at->format('d M Y, H:i') }}</td></tr>
                    @if($order->notes)
                    <tr><td class="text-muted">Catatan</td><td>{{ $order->notes }}</td></tr>
                    @endif
                </table>
            </div>
        </div>
            ...
            <div class="card-body px-4 pb-4">
                <table class="table table-borderless text-body order-detail-table">
                    @foreach($ptProLabels as $key => $label)
                        ...
                        <tr><td width="40%" class="text-muted">{{ $label }}</td>
                            <td class="fw-semibold">{{ $order->form_data[$key] }}</td></tr>
                        @endif
                    @endforeach
                </table>
            </div>
    ...
    <div class="col-md-4">
        ...
        @php
            ...
            $isEligible      = RoomBenefit::isEligiblePackage($rawSlug) || RoomBenefit::isEligiblePackage($serviceNameRaw);
