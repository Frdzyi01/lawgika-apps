                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-hover">
                        ...
                        <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex flex-column gap-1">
                                            ...
                                            @if ($booking->status === 'pending')
                                                <form
                                                    action="{{ url('admin/meeting-room/' . $booking->id . '/benefit-approve') }}"
                                                    method="POST">
                                                    @csrf
                                                    <button class="btn btn-sm btn-success w-100"
                                                        onclick="return confirm('Setujui reservasi ini?')"><i
                                                            class="fas fa-check"></i> Approve</button>
                                                </form>
                                                <form
                                                    action="{{ url('admin/meeting-room/' . $booking->id . '/benefit-reject') }}"
                                                    method="POST">
                                                    @csrf
                                                    <button class="btn btn-sm btn-danger w-100"
                                                        onclick="return confirm('Tolak reservasi ini?')"><i
                                                            class="fas fa-times"></i> Reject</button>
                                                </form>
                                            @elseif($booking->status === 'approved' || $booking->status === 'paused')
                                                <form action="{{ url('admin/meeting-room/' . $booking->id . '/checkin') }}"
                                                    method="POST">
                                                    @csrf
                                                    <button class="btn btn-sm btn-success w-100"
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="text-center text-muted">Belum ada reservasi benefit meeting
                                        room.</td>
                                </tr>
                            @endforelse
                        </tbody>
