...
<div class="container-fluid">
    ...
    @if(isset($benefits) && $benefits->count() > 0)
        @foreach($benefits as $ben)
            ...
            @endphp
            <div class="card shadow-sm border-0 mb-4" style="border-radius:16px; border-left: 5px solid #4e0516 !important; background:#ffffff;">
                <div class="card-body p-4">
                    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                        <div>
                            @php
                                $isBenefitFromPT = !empty($ben->order_id) || ($ben->total_minutes <= 12 * 60);
                                $badgeLabel = $isBenefitFromPT ? 'Paket Benefit Studio Podcast (12 Jam)' : 'Paket Podcast Room (20 Jam)';
                            @endphp
                            <span class="badge bg-danger text-white px-2.5 py-1 rounded-pill mb-1" style="font-size:0.75rem;">
                            ...
                            <h5 class="fw-bold mb-0 text-dark" style="font-size:1.15rem;">
                                <i class="fa-solid fa-microphone-lines me-2 text-danger"></i> {{ $ben->paket }}
                            </h5>
                        </div>
                    ...
                    <div class="alert alert-info py-2 px-3 mt-3 mb-0 small rounded-3 d-flex align-items-center gap-2">
                        ...
                        <div>
                            <strong>Alur Penggunaan:</strong> Klik tombol <strong>"Ajukan Reservasi Baru"</strong> untuk memilih jadwal studio dan durasi. Setelah reservasi diverifikasi &amp; disetujui admin, sesi akan berstatus <em>Siap Check In</em> dan kuota otomatis dipotong saat Check Out selesai.
                        </div>
                    </div>
                </div>
            </div>
                ...
                <table class="table table-bordered table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                @if($b->source_type === 'benefit' || $b->benefit_id || $b->duration == 12)
                                    <span class="badge bg-info text-dark">Paket Benefit (12 Jam)</span>
                                    @if($b->benefit)
                                    <br><small class="text-muted">{{ $b->benefit->paket }}</small>
                                    @endif
                                @elseif($b->duration == 20)
                                    <span class="badge bg-danger">Paket Podcast Room (20 Jam)</span>
                                @else
                                    <span class="badge bg-primary">Paket Podcast Room Per Jam ({{ $b->duration }} Jam)</span>
                                @endif
                            </td>
                            <td>
                                @if($b->is_expired)
